...
from dataset.constants import BOOKS_WITH_DATES, RANDOM_STATE


DEFAULT_TEST_SIZE: float = 0.25
"""Default value of fraction of the dataset to include in test split."""

PATH_TO_MAPPINGS: str = BOOKS_WITH_DATES
...
def drop_additional_columns(
    ...
    drop_name_column: bool = True,
) -> pd.DataFrame:

    if drop_book_column:
        X = X.drop(columns=["book"])
...
def split_dataframe(
    ...
    X_train = drop_additional_columns(X_train, drop_book_column, drop_name_column)
    X_test = drop_additional_columns(X_test, drop_book_column, drop_name_column)

    return X_train, X_test, y_train.to_numpy().copy(), y_test.to_numpy().copy()
...
def get_time_series_split_dataframe(
    ...
    result = []
    for _, row in books_per_year.iterrows():
...
def split_levels_column(
    ...
    X = drop_additional_columns(X, drop_book_column, drop_name_column)
