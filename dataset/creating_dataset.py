...
from dataset.constants import ORDERED_CHARACTERISTICS_FULL
from dataset.splitting_dataset import load_csv_mapping_file
...
def make_get_characteristic_from_list(characteristic_name: str) -> callable:
    def get_value(x):
        return get_characteristic_from_list(x, characteristic_name)
...
def extract_and_assign_chars(
    ...
    for char in char_group:
        ...
        get_value = make_get_characteristic_from_list(characteristic_name)
...
def get_max_melee_bonus_damage(
    ...
    """
    melee = [i["system"] for i in items_list if i["type"] == "melee"]

    if weapon_type == "melee":
        melee = [i for i in melee if i.get("range") is None]
    elif weapon_type == "ranged":
        melee = [i for i in melee if i.get("range") is not None]
...
def count_immunities(
    x: np.float_ | Sequence,
):  # np.float because it is either nan or sequence
    return 0 if x is np.nan else len(x)
...
def sort_preprocessed_data(bestiary: pd.DataFrame) -> pd.DataFrame:
    bestiary = bestiary[
        [
            col
            for col in ["level", "book"] + ORDERED_CHARACTERISTICS_FULL + ["name"]
...
def remove_older_monsters_duplicates(df: pd.DataFrame) -> pd.DataFrame:
    unique_monsters_from_all_sources = len(df[CHARACTERISTICS_RENAME["name"]].unique())

    books_mapping = list(
        load_csv_mapping_file().sort_values(by=["date"], ascending=False)["book"]
    )
    # filter books without dates
    df = df[df[CHARACTERISTICS_RENAME["book"]].isin(books_mapping)]
    monsters_with_known_source_date = len(df[CHARACTERISTICS_RENAME["name"]].unique())

    df[CHARACTERISTICS_RENAME["book"]] = pd.Categorical(
        df[CHARACTERISTICS_RENAME["book"]],
        books_mapping,
    )
    df = df.sort_values(by=[CHARACTERISTICS_RENAME["book"]])

    df = df.drop_duplicates(subset=["name"])
    monsters_no_duplicats = df[CHARACTERISTICS_RENAME["name"]].shape[0]

    print(f"""Summary
        All unique monsters from JSON files: {unique_monsters_from_all_sources}
        Monsters from labeled books: {monsters_with_known_source_date}
        Final monsters number: {monsters_no_duplicats}
    """)
...
def preprocess_data(bestiary: pd.DataFrame, characteristics: list[str]) -> pd.DataFrame:
    ...
    bestiary = remove_older_monsters_duplicates(bestiary)
...
def load_all_files(folder: str) -> pd.DataFrame:
    ...
    return load_data(files_list)


def load_and_preprocess_data(
    characteristics: list[str],
    paths_to_books: Optional[list[str]] = None,
    folder: Optional[str] = None,
    chosen_books: Optional[list[str]] = None,
) -> pd.DataFrame:
    """
    ...
    if paths_to_books:
        ...
        bestiary = load_all_files(folder)
    else:
        raise ValueError("Pass paths_to_books or folder")

    if chosen_books:
        bestiary = bestiary[bestiary[CHARACTERISTICS_RENAME["book"]].isin(chosen_books)]

    return preprocess_data(bestiary, characteristics)


def min_max_scale_data(df: pd.DataFrame) -> pd.DataFrame:
    columns = [col for col in df.columns if col not in ["book", "level", "name"]]
    scaler = MinMaxScaler()
